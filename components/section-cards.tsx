 ...
 * (`/health` 도 계약에 있는 표면이고 운영자가 가장 먼저 묻는 것이다).
 *
 * 순수 렌더링이다 - 카운트 셋과 상태 하나를 모두 props 로 받는다. 이 컴포넌트
 * 자신은 아무 것도 fetch 하지 않는다 - `app/(admin)/page.tsx` 가 세 카운트
 * 요청과 헬스 확인을 모아 `headers()` 를 부르는 자리를 화면 하나로 유지한다
 * (`app/(admin)/examples/list.ts` 의 listRequest 와 같은 이유).
 *
