              ...
              content={
                <ChartTooltipContent
                  labelFormatter={(value) => {
                    // `ChartTooltipContent`의 `labelFormatter` 는 recharts 의
                    // 여러 콘텐츠 렌더러 시그니처를 합친 오버로드 타입이라
                    // 매개변수를 `string` 으로 직접 선언하면 그 합쳐진 타입에
                    // 대입할 수 없다(계약상 `ReactNode`/`any`도 와야 한다).
                    // 이 차트의 X축은 항상 `date` 문자열 키를 쓰므로, 여기서만
                    // 좁혀 캐스팅한다.
