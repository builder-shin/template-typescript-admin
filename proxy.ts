 ...
 *
 * **`/register` 도 이 패턴에 걸려 보호된다 - 가입 화면에는 이것이 안전장치가
 * 아니라 함정이다.** 다른 어떤 화면과도 다르게, 가입 화면은 **세션이 없는
 * 사람이 와야 하는** 유일한 화면이다(로그인해야 계정을 만들 수 있다면
 * 첫 계정을 아무도 못 만든다). 이 저장소는 가입 화면을 만들지 않기로
 * 했다 - 운영자는 스스로 가입하지 않고, 첫 운영자는 `scripts/seed-operator.ts`
 * (백엔드 시드)가 만든다. `lib/auth/credentials.ts` 의 `signUp`·
 * `signUpThenSignIn` 등 가입 로직 자체는 이미 존재한다(다른 프론트엔드
 * 템플릿과 공유하는 인증 코어라 지우지 않았을 뿐이다 - `app/(auth)/actions.ts`
 * 머리말, `lib/AGENTS.md` 의 "복사해 온 코어 안에서, 이 저장소가 실제로
 * 쓰지 않는 자리" 절 참고) - 그래서 나중에 누군가 그 로직에 `/register`
 * 화면만 붙이는 것은 그럴듯한 작업으로 보인다.
 *
 * **그 작업을 이 패턴을 그대로 둔 채로 하면 화면은 항상 켜져 있는데 아무도
 * 가입할 수 없다.** `isProtectedPath('/register')` 가 참이라 세션 없는
 * 방문자는 그 화면에 닿기도 전에 `/login` 으로 튕긴다 - 화면을 붙인
 * 사람이 로그인한 채로(세션이 있는 채로) 확인하면 멀쩡히 렌더되므로, 이
 * 구멍은 실제 익명 방문자가 처음 시도할 때까지 드러나지 않는다. 가입
 * 화면을 붙이려면 **이 패턴에 `/register` 예외를 반드시 추가해야 한다**
 * (예: `/^(?!\/login$)(?!\/register$).*$/`) - 이 문단이 그 사실을 남긴다.
 */
