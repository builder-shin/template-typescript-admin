...
이 디렉터리는 어떤 자원 이름도 몰라야 한다(스펙 4장). `lib/resources/`가
...
· `'relationships'` · `page[...]`), 연산자 이름, 스펙 9.2 표가 지정한 오류
**코드**(문구가 아니다), 미디어 타입, HTTP 헤더 이름이다.
...
상황이라 프론트가 문구를 가질 수밖에 없다(스펙 9.2). 이 세 코드의 `detail`은
...
달리 화면이 아니라 `app/error.tsx`가 받는다(스펙 9.2). 분류
